...
class WisataTopAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class ListViewHolder(val binding: ItemsPencarianTeratasBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(wisata: WisataData, callback: OnItemClickCallback?) {
            binding.apply {
                tvLocationName.text = wisata.nama
                tvLocationAddress.text = wisata.lokasi
                tvRating.text = wisata.rating
                ivLocationImage.loadImage(wisata.foto)
            }
